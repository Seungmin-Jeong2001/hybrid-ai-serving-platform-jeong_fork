...
- `GPU_WORKER_COUNT`: 기본 `0`. `1`로 설정하면 `g1.large` GPU passthrough flavor를 쓰는 전용
  GPU worker VM을 생성합니다.
- `GPU_WORKER_IMAGE_NAME`: 기본 `ubuntu-22.04`
- `GPU_WORKER_FLAVOR_NAME`: 기본 `g1.large`
- `GITLAB_FLAVOR_NAME`: 기본 `m1.large`. local DevStack apply에서는 CPU를 4 vCPU로 유지하고 RAM만
  16GB로 올린 전용 `ha.m1.gitlab` flavor를 자동 생성해 사용합니다.
...
처리합니다.
GitLab runtime default는 4 vCPU 기준으로 `puma['worker_processes'] = 1`,
`puma['max_threads'] = 3`, `sidekiq['concurrency'] = 1`이며, OOM 완충용 4GB swap을 생성합니다.
